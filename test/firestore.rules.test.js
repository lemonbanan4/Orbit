...
describe("Orbit Firestore Security Rules", () => {
  ...
  it("Allows a weeklyTarget of 1..7, denies out-of-range or wrong type", async () => {
    const db = testEnv.authenticatedContext("user123").firestore();
    const habitDoc = db
      .collection("users")
      .doc("user123")
    ...
    await testEnv.withSecurityRulesDisabled(async (context) => {
      ...
      await context
        ...
        .collection("habits")
        .doc("habit1")
        .set({ title: "Gym", routine: "Morning", completedDays: 0, totalDays: 0 });
    });

    await assertSucceeds(habitDoc.update({ weeklyTarget: 3 }));
    await assertFails(habitDoc.update({ weeklyTarget: 0 }));
    await assertFails(habitDoc.update({ weeklyTarget: 8 }));
    await assertFails(habitDoc.update({ weeklyTarget: "3" }));
  });

  it("Allows a valid healthMetric/healthTarget, denies unknown metric or bad target", async () => {
    const db = testEnv.authenticatedContext("user123").firestore();
    const habitDoc = db
      .collection("users")
      .doc("user123")
    ...
    await testEnv.withSecurityRulesDisabled(async (context) => {
      ...
      await context
        ...
        .collection("habits")
        .doc("habit1")
        .set({ title: "Walk", routine: "Morning", completedDays: 0, totalDays: 0 });
    });

    await assertSucceeds(habitDoc.update({ healthMetric: "steps", healthTarget: 10000 }));
    await assertSucceeds(habitDoc.update({ healthMetric: "workout_minutes", healthTarget: 30 }));
    await assertFails(habitDoc.update({ healthMetric: "sleep" }));
    await assertFails(habitDoc.update({ healthTarget: 0 }));
