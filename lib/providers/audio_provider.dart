...
class AudioProvider extends ChangeNotifier {
  ...
  AudioProvider() {
    // onError callbacks prevent an unhandled-stream-error from propagating up
    // to the global PlatformDispatcher handler (Crashlytics) every time the
    // emulator's c2.android.mp3 codec stack throws BAD_INDEX.
    _positionSubscription = _audioPlayer.positionStream.listen(
      ...
      onError: (Object e, StackTrace st) =>
          debugPrint('[AudioProvider] positionStream error: $e'),
    );
    _durationSubscription = _audioPlayer.durationStream.listen(
      ...
      onError: (Object e, StackTrace st) =>
          debugPrint('[AudioProvider] durationStream error: $e'),
    );
    _playerStateSubscription = _audioPlayer.playerStateStream.listen(
      ...
      onError: (Object e, StackTrace st) =>
          debugPrint('[AudioProvider] playerStateStream error: $e'),
    );
  }
  ...
  Future<void> play(String url, {String? title, String? subtitle, String? artUrl}) async {
    if (_currentUrl != url) {
      ...
      final mediaItem = MediaItem(
        ...
        // Guard: just_audio_background passes artUri to flutter_cache_manager which
        // performs an HTTP fetch. Local asset:// URIs crash with "No host specified".
        // Only pass HTTP(S) URLs; fall back to a remote image otherwise.
        artUri: (_currentArtUrl?.startsWith('http') == true)
            ? Uri.parse(_currentArtUrl!)
            : Uri.parse('https://images.unsplash.com/photo-1444703686981-a3abbc4d4fe3?q=80&w=1000&auto=format&fit=crop'),
      );
